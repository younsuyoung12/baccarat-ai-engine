...
engine_state.py
====================================================
Baccarat Predictor AI Engine – 엔진 상태 저장/복원 모듈

역할
- Big Road(원본), 패턴 히스토리, 메타러닝 통계, Road Leader 상태를
  JSON 파일로 저장/복원한다.
- 로드 파생 데이터(Big Eye/Small/Cockroach/매트릭스 등)는 복원 후 recompute_all_roads()로
  무결성 있게 재계산한다.

변경 요약 (2025-12-24)
----------------------------------------------------
1) ✅ Deadlock 제거: 트레이딩 준비도에서 Leader 통계를 "차단 조건"에서 "워밍업 조건"으로 변경
   ...
   - 목적: WARMUP(leader 미학습) 상태가 영원히 풀리지 않는 순환 조건 제거
2) 폴백 금지 정책 유지
   - 스키마/타입/필수키 무결성 검증은 그대로 엄격(누락/타입 위반 시 즉시 예외)
----------------------------------------------------

(기존 변경 요약 2025-12-23)
----------------------------------------------------
1) features_entry.py 의존 제거(옵션 A)
   - from features_entry import ... 완전 삭제
   - EntryState(난기류/모멘텀) 저장/복원 로직 제거
2) 상태 파일 스키마 버전 업
   - SCHEMA_VERSION = 2
   - load는 schema_version 1/2를 "명시적으로" 지원
     (v1의 turbulence_counter/entry_momentum은 타입 검증만 하고 사용하지 않음)
3) readiness 정책 유지
   - load 단계에서 readiness 미달로 리셋 루프 유발 금지
   - 베팅 직전 assert_ready_or_raise()에서만 차단
----------------------------------------------------
"""
...
def get_trade_readiness() -> Tuple[bool, str]:
    ...
    # ✅ 차단하지 않고, READY이되 워밍업 사유를 이유 문자열로만 남긴다.
    parts = []
...
def save_engine_state(*, last_decision: Dict[str, Any] | None = None) -> None:
    ...
    state: Dict[str, Any] = {
        ...
        "micro_learning": meta_learning.micro_learning_stats,
        "meta_learning": meta_learning.meta_learning_stats,
        ...
        # 기존 키는 '베팅 준비도'로 유지(단, leader warmup이어도 True가 될 수 있음)
...
def load_engine_state(*, strict_ready: bool = False) -> None:
    ...
    # v1 레거시 필드는 '검증만' 하고 사용하지 않는다(옵션 A)
    if ver_i == 1:
    ...
    micro = _require_dict(state, "micro_learning")
    meta = _require_dict(state, "meta_learning")
    ...
    meta_learning.micro_learning_stats = micro
    meta_learning.meta_learning_stats = meta
